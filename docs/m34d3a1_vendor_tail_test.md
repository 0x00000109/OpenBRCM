...
ob_d3a1_test(hw):
  ...
  ob_d3a1_fifo_fixup(hw)                -> sub_67efd equivalent
  ob_d3a1_t1(hw)                        -> T1
...
## 8. `switch_macfreq` (BCM4352 path)

`ob_d3a1_switch_macfreq()` reads the PMU PLL (chipcommon `0x660/0x664`) and
applies the recovered `si_pmu_get_bb_vcofreq` 0x4352 arithmetic; then
`ob_d3a1_tsf_frac(vco) = ((u64)0x80000000 << 32 | 0x3a9) / vco` (the native
64-bit equivalent of the vendor `bcm_uint64_divide`) and writes
`0x62e = frac[15:0]`, `0x630 = frac[31:16]`. It logs the source VCO value and
the derived words; readback is diagnostic only.
...
- `tests/host/ob_d3a1_test.c` (PASS): mode conflict (5-mode), D3A1 order model
  ...
  absent `btc_params`/`btc_flags` skip, `switch_macfreq` math, DMA
  teardown/fatal lifecycle.
...
| module | `openbrcm.ko` |
| size | 4127081 B |
| sha256 | `045448c983786dc61a6b80fcb953b54282b3236353abffbdf98cf27f9629b9cb` |
| vermagic | `7.0.0-34-generic SMP preempt mod_unload modversions` |
| srcversion | `15F0610DFED7C843198BFE3` |
...
2. The `switch_macfreq` 64-bit PLL3 fold (`(p2>>4)&7 != 0`) is a documented
   best-effort port of the vendor `bcm_uint64_multiple_add`/`bcm_uint64_divide`
   arithmetic; the primary formula and the final divide are exact. No hardware
   validation is possible in this task.
3. SRL/LRL/SFBL/LFBL use the vendor/upstream attach defaults (SRL 7, LRL 6,
   SFBL 3, LFBL 2) because the isolated test has no NVRAM retry-limit source.
4. The symbolic name of the SHM `0x78c/0x78e/0x790` slots remains UNKNOWN
   (value/source proven).
