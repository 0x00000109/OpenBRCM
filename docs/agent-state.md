...
M3.4D1 isolated `fw_validate_only=1` runtime PASS (see above).
...
## Current next action
M3.4D2A **hardware execution** (single load with `ucode_test_only=1`). This is
**NOT approved/executed**. It requires explicit human approval. No agent may
run it autonomously.
