...
It:
  ...
  * decodes ONLY fields whose rev11 raw offset is supplied as provenance
    (``--offsets``); it never guesses offsets -- without a proven table the
    fields are reported ``null`` with ``provenance: "unknown"``;
  * reproduces the vendor ``wlc_antsel_attach`` ``antsel_type`` function
    exactly and maps it to the D3B MHF3 value.

The rev11 raw offsets are the remaining static blocker for MHF3; this tool is
the consumer side and is ready for them, but it will not invent them.  See
...
# --------------------------------------------------------------------------
# Field decoding (provenance table only)
# --------------------------------------------------------------------------
# Proven rev11 raw offsets. EMPTY: the exact rev11 layout is not yet recovered
# from the vendor srom_parsecis parser (see docs/m34d3b_sprom_evidence.md).
# Nothing here may be guessed; supply a proven table with --offsets.
PROVEN_OFFSETS = {}
...
def decode_field(words, name, spec):
    ...
    raw = _read_le(words, bo, width)
    value = (raw >> shift) & ((1 << width) - 1)
    nbytes = (width + 7) // 8
    word_indices = [(bo + 2 * i) // 2 for i in range(nbytes // 2)]
    return {
        "field": name,
        "byte_offset": bo,
...
def main(argv=None):
    ...
    if args.decode:
        ...
        # Only compute antsel_type/MHF3 if all inputs decoded.
        need = ("boardtype", "boardflags", "antswitch", "aa2g", "aa5g")
        if all(n in fields for n in need):
            atype, avail = antsel_type(*(fields[n]["value"] for n in need))
